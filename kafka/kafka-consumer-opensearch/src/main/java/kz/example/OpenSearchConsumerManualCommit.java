...
public class OpenSearchConsumerManualCommit {
  ...
  static final Logger logger = LoggerFactory.getLogger(OpenSearchConsumerManualCommit.class);
  ...
  public static void main(String[] args) throws IOException {
    ...
    try (openSearchClient; consumer) {
      ...
      while (true) {
        ...
        logger.info("offset now (before manual commit): ");
        consumer.assignment().forEach(x -> System.out.println("" + consumer.currentLag(x).orElse(0)));

        consumer.commitSync();

        logger.info("offset now (after commit): ");
        consumer.assignment().forEach(x -> System.out.println("" + consumer.currentLag(x).orElse(0)));
