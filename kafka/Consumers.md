...
- Консюмеры в группе обращаются к consumer group coordinator-у
- как проверяют жизнь?
- механизм поллинга
- механизм отправки heartbeats
- консюмеры настроены к быстрому процессингу данных, поэтому часто поллятся для поиска новых входных данных

### Consumer heartbeat thread - отдельный поток отправляющий брокеру состояние о жизни
- `heartbeat.interval.ms`
- поток сердцебиения консюмера (по умолчанию длится 3 сек)
- желательно устанавливать как 1/3 от таймаута сессии `session.timeout.ms` (по умолчанию 45 сек с кафки 3.0, до нее было 10 сек)
- чем ближе стоит `session.timeout.ms`, тем быстрее брокер обнаружит упавший консюмер. Но берегитесь ЛОЖНОГО упадка консюмера из-за того что моргнула сеть

## Consumer Poll thread - `max.poll.interval.ms`
- время между двумя вызовами метода poll в консюмере
- если больше данного времени - консюмер считается мертвым и выходит из группы
- !!! ЭТОТ МЕХАНИЗМ НУЖЕН для того случаю когда консюмер перегружен и не может справляться с нагрузкой или обрабатывает данные очень медленно
- !!! `like consumer in stuck` or data processing issue

### `max.poll.records` - сколько максимально рекордов можно прочитать за запрос
- нужно уменьшить если сообщения большие по размеру (чтобы уменьшить потребление RAM оперативной памяти)
- или увеличить если позволяет оперативка и сообщения слишком маленькие, их кол-во можно увеличить (например, вместо тысяч машин на пробке один автобус*)
- данную настройку можно мониторить, посмотрев по сколько запросов копится в пачке

### `fetch.min.bytes` - минимальный размер сообщения (рекордов) в запросе
### `fetch.max.wait.ms=500ms` - если в пачке нет данных размера `fetch.min.bytes`, 
максимальное время, на которое заблокируется запрос до возвращения ответа. Иногда эффективней заблокировать поток, нежели сразу передавать по сети


### `max.partition.fetch.bytes=1MB` - макс кол-во данных, читаемых с одной партиции. Если вы читаете со 100 партиций одновременно, вам нужно больше RAM
### `fetch.max.bytes=55MB` - макс кол-во данных, возвращаемых за запрос. Можно увеличить чтобы позволить консюмеру читать больше данных за запрос. 
- увеличивайте две настройки выше если пачки копятся быстрее и консюмер оперативно справляется

### Consumer replica fetching - чтение из ближайшего датацентра
- by default consumer reads from leader
- since kafka 2.4 possible to read from the closest replica (when you have many data centers)
- это НАСТРОЙКА БРОКЕРА, затем нужно установить настройку консюмера `cleint.rack`
- `replica.selector.class` should be `RackAwareReplicaSelection`
- `rack.id` - set here data center ID
