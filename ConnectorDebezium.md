...
### Минусы и ограничения Дебезиума
...
- Дебезиуму нужна конкретная машина где он будет читать WAL лог
- Нельзя  уккзать несаолько хостов дебезиуму, перед ними должен стоять балансировщик
- Если коннектор упадет, логи могут копиться в WAL и набухнуть пока мы их не прочитаем
- При первом запуске кафка делсет `serializable` транзакцию чтобы получить снапшот табдицы


### CDC Debezium не помог ТБанку?
- сложность настройки и управления (общий инструмент не подходит под конкретный сценарий. Сложность работы с кастомными avro схемами)
- неуправляемые задержки (нет гарантии своевременной доставки)
- консистентность (транзакции в разных таблицах)
